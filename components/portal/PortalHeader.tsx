...
export function PortalHeader({ content, initialUser, posts }: Props) {
  ...
  const socialLinks: Array<{ href: string; title: string; tone: "telegram" | "line" }> = [];
  const telegramHref = content.telegram.ctaHref?.trim();
  const lineHref = content.line.ctaHref?.trim();
  if (telegramHref) socialLinks.push({ href: telegramHref, title: "Telegram", tone: "telegram" });
  if (lineHref) socialLinks.push({ href: lineHref, title: "LINE", tone: "line" });
