  ...
  if (posts.length === 0) {
    return (
      <div className="space-y-5">
        <div className="rounded-2xl border border-dashed border-white/10 bg-white/[0.02] px-6 py-10 text-center backdrop-blur-md">
...
function MerchantListCard({ post }: { post: PortalPostRow }) {
  ...
  return (
    <Link
      href={`/post/${post.id}`}
      className="group cursor-pointer overflow-hidden rounded-2xl border border-white/[0.06] bg-white/[0.03] p-3 backdrop-blur-xl transition-all duration-300 hover:border-[#FFD700]/15 hover:bg-white/[0.06] hover:shadow-[0_12px_40px_-8px_rgba(0,0,0,0.5),0_0_0_1px_rgba(255,215,0,0.06)] block"
      style={{
    ...
    >
      ...
      <div className="mb-3 flex items-start gap-3">
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[11px] text-zinc-600">剛剛</span>
          </div>
      ...
      <div className="grid grid-cols-3 gap-1.5 overflow-hidden rounded-xl">
        <div className="col-span-2 relative aspect-[3/4] overflow-hidden rounded-lg bg-white/5">
          {mainImage ? (
        ...
        <div className="col-span-1 flex flex-col gap-1.5">
          ...
          {sideImages.length < 2 && (
            <div className={`relative flex-1 overflow-hidden rounded-lg bg-white/[0.03] ${sideImages.length === 0 ? 'hidden' : ''}`}></div>
          )}
        </div>
      </div>
    </Link>
